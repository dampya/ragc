...
// Handles incoming keypress data from DSKY frontend
fn handle_stream_input(stream: &mut TcpStream, keypress_tx: &Sender<u16>) {
    loop {
        let mut buf = [0; 4];
        match stream.read_exact(&mut buf) {
            Ok(_x) => match parse_dsky_packet(buf) {
...
// Sends outgoing DSKY display updates
...
// Handles blinking segments by manipulating flash flags over time
fn flashing_thread(flash_rx: Receiver<u16>, dsky_tx: Sender<[u8; 4]>) {
    ...
    loop {
        ...
        if elapsed % 1000 < 750 {
            ...
            dsky_tx.send(generate_dsky_packet(0o0163, value)).unwrap();
        } else {
...
// Starts DSKY server and handles each client serially
...
impl DskyDisplay {
    ...
    pub fn set_dsalmout_flags(&mut self, flags: u16) {
        if self.last_dsalmout != flags {
            ...
            self.dsky_tx
                .send(generate_dsky_packet(0o11, flags))
                .unwrap();
    ...
    pub fn set_channel_dsky_value(&mut self, val: u16) {
        ...
        self.dsky_tx.send(generate_dsky_packet(0o10, val)).unwrap();
...
impl ragc_core::memory::periph::IoPeriph for DskyDisplay {
    fn read(&self, channel_idx: usize) -> u16 {
        match channel_idx {
            ragc_core::constants::ports::CHANNEL_MNKEYIN => self.read_keypress(),
            ragc_core::constants::ports::CHANNEL_CHAN30 => 0o77777,
            ragc_core::constants::ports::CHANNEL_CHAN31 => 0o77777,
            ragc_core::constants::ports::CHANNEL_CHAN32 => self.read_proceed_flag(),
            ragc_core::constants::ports::CHANNEL_CHAN33 => 0o77777,
    ...
    fn write(&mut self, channel_idx: usize, value: u16) {
        match channel_idx {
            ragc_core::constants::ports::CHANNEL_DSKY => self.set_channel_dsky_value(value),
            ragc_core::constants::ports::CHANNEL_DSALMOUT => self.set_dsalmout_flags(value),
            ragc_core::constants::ports::CHANNEL_CHAN13 => {
                self.set_channel_value(channel_idx, value)
            }
            0o163 => self.set_channel_value(channel_idx, value),
            _ => {}
        }
    }
    ...
    fn is_interrupt(&mut self) -> u16 {
        if self.keypress.len() > 0 {
            ...
            (1 << ragc_core::constants::registers::INTERRUPT_KEYPRESS1) as u16
        } else {
            0
        }
    }
